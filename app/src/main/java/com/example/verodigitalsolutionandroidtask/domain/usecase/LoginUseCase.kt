...
class LoginUseCase @Inject constructor(private val authRepository: AuthRepository, private val authDataStore: AuthDataStore) {

    suspend operator fun invoke(username: String, password: String): LoginUiState {
        try {
            val result =  authRepository.login(username, password)
            authDataStore.saveAccessToken(result.oauth.access_token)
